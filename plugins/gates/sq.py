...
async def premium(_, m: Message):
    ...
    product_link = "https://www.comeasyouare.com/collections/lubricants/products/pink-silicone-pillow-pack"
    ...
    if not r1 or not varient_id: return await m.reply('Error1')
    ...
    r2 = r.post(f'https://{webname}/cart/add',data =b_data)
    if not r2: return await m.reply('Error2')
...
'checkout': 'Check Out'
    ...
    session_token = buscarEnT(r3.text, '&quot;tracking_visit&quot;:&quot;','&quot')
    shop_id = buscarEnT(r3.text, 'shopId&quot;:', ',')
    if not shop_id: return await m.reply('Error shop_id')
    if not r3: return await m.reply('Error3')
    if not session_token: return await m.reply(r3.url)
    #time.sleep(3)
    r4_payload = {
	"metadata": {
		"event_created_at_ms": 1672206935128,
		"event_sent_at_ms": 1672206935128
	},
	"payload": {
		"duration": 10486,
		"navigation_start": 1672206924642,
		"page_type": "cart",
		"session_token": session_token,
		"shop_id": shop_id,
		"url": "https://www.comeasyouare.com/cart"
	},
	"schema_id": "online_store_buyer_site_abandonment/1.1"
}
    
    r4 = r.post("https://monorail-edge.shopifysvc.com/v1/produce", data=r4_payload)
    print(r4.status_code)
    r5 = r.get("https://www.comeasyouare.com/checkouts/c/734f633bdc1f1d060edd8257432e761a?_ga=undefined")
    if not r5: return await m.reply('Error')
    print(r5.text)
      
    return await m.reply(shop_id)
    checkout_url = r3.url
    time1 = time.time()
